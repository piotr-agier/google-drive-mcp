...
- **resources:** add `GOOGLE_DRIVE_MCP_DISABLE_RESOURCES` env var (and `--no-resources` flag) to opt out of the MCP resource protocol (`gdrive:///` listing/reading); tools stay available. For tools-only clients or clients that hang enumerating a large Drive ([#115](https://github.com/piotr-agier/google-drive-mcp/issues/115), [#NNN](https://github.com/piotr-agier/google-drive-mcp/pull/NNN))

### Bug Fixes

- **resources:** raise the `resources/list` page size from 10 to 1000 (Drive API max) so clients that eagerly enumerate the entire Drive (e.g. Gemini CLI) no longer hang during initialization ([#111](https://github.com/piotr-agier/google-drive-mcp/issues/111), [#NNN](https://github.com/piotr-agier/google-drive-mcp/pull/NNN))
