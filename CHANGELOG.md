...
- **docs:** `docs/tools.md` told callers `addSheet` takes `sheetTitle`; the tool's own schema takes `title`, required, so following the documentation produced a validation error instead of adding a sheet. Nothing tied the two together, and this class of drift between the prose and the schema it duplicates has bitten the project before, so a new test now reads every parameter bullet in `docs/tools.md` — the backticked name(s) leading a bullet, before the colon — and asserts each one is a real key in that tool's `inputSchema.properties`, catching this shape of bug at build time rather than leaving a caller to hit the validation error first. The reverse direction (every schema parameter documented) is deliberately out of scope: dozens of existing parameters go undocumented today, and closing that gap is a documentation project, not a gate
