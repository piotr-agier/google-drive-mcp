...
- **docs:** `docs/tools.md` told callers `addSheet` takes `sheetTitle`; the tool's own schema takes `title`, required, so following the documentation produced a validation error instead of adding a sheet. Both tools now carry their own parameter bullets, since the alias takes `title` where `addSpreadsheetSheet` takes `sheetTitle`. Nothing tied the two together, and this class of drift between the prose and the schema it duplicates has bitten the project before, so a new test now reads every parameter bullet in `docs/tools.md` — the backticked name(s) leading a bullet, before the colon — and asserts each one is a real key in that tool's `inputSchema.properties`, catching this shape of bug at build time rather than leaving a caller to hit the validation error first ([#222](https://github.com/piotr-agier/google-drive-mcp/pull/222), [#223](https://github.com/piotr-agier/google-drive-mcp/pull/223))

### Changed

- **reliability:** every Google API call in the five tool files now runs under `withRetry`, the per-attempt timeout and backoff helper this server already shipped and barely used. Across roughly 181 call sites — including `getGoogleSheetCells` and `updateGoogleSheetIfUnchanged`, added to `sheets.ts` after this work started — it appeared exactly once beforehand, in `docs.ts`; everywhere else a handler awaited a bare googleapis promise with no deadline and no cancellation, so a stalled Google response held the MCP request open indefinitely — and on a hosted deployment held a server concurrency slot shared with every other consumer along with it. This is the same failure the 2.8.0 token-refresh work fixed for OAuth refresh; the tool call sites were never given the same treatment. Reads (90 sites) get the full policy, timeout plus the existing bounded backoff, because repeating a read is safe. Writes and anything else with a side effect (91 sites) get the timeout **only**, via `retryMax: 0`: they gain hang protection without gaining a retry they never had, because the policy counts `ECONNRESET` as retryable and a reset can be raised after a server has accepted a request, so a blind retry of a non-idempotent write could double-apply it — the same reasoning that already keeps HTTP 500 and 502 out of the retryable set. The deadline does real work rather than merely abandoning the wait: `withRetry` aborts the attempt through the `AbortSignal` it hands its callback, and every wrapped call forwards it as gaxios `{ signal }`. A metadata read a write tool makes to resolve a sheet id, a grid range, or a `textToFind` index is classified as the read it is. `createGoogleDoc`'s initial-content `batchUpdate`, the one pre-existing call, is left exactly as it was. A stalled *read* is not a single 120s wait: `TimeoutError` carries `ETIMEDOUT`, which is retryable, so the call is retried `retryMax` times before the error surfaces — four attempts of 120s plus backoff by default, roughly eight minutes, not two. A stalled write still fails after one 120s attempt, since `retryMax: 0` skips the retry — except the eight calls that carry a `media` upload body (`createTextFile`, `updateTextFile`, `uploadFile`, `uploadPdfWithSplit`, `createDocFromHTML`, and `restoreRevision`'s rewrite), which get a 30-minute deadline instead of the 120s default, since a non-stream gaxios promise settles only once the request body has fully transferred, so the general deadline would otherwise bound the upload rather than the wait for a response. Ten call sites in shared helper modules outside the five tool files (`driveImageUpload`, `text-content`, `download-file`, `auth/identity`), plus seven bare Drive calls in `src/index.ts`'s resource-read context helpers (`resolveFolderId`, `checkFileExists`, and the search/read/export helpers), are not covered yet. No tool, parameter, or configuration option changes: the knobs already existed with their defaults (`--api-timeout`, 120s; `--retry-max`; `--retry-base-delay`), a healthy call behaves exactly as before, and `--api-timeout=0` restores the previous unbounded behaviour ([#218](https://github.com/piotr-agier/google-drive-mcp/pull/218))
