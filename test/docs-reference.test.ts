...
describe('Documentation reference', () => {
  ...
  //
  // A second shape also counts as a claim: a short prose label carrying one
  // parenthetical annotation, e.g. `Target (use one):`, or the literal prefix
  // `Provide either`, immediately followed by two or more backticked names
  // joined by `OR`/`or`, `+`, `,`, `/`, or `with`. Each name may carry its
  // own trailing parenthetical, e.g. `` `startIndex` (1-based, inclusive) ``;
  // that annotation is skipped over, and any backticks inside it (enum
  // members such as `` `position` (`before`/`after`) ``) are not claims.
  // `` Target (use one): `index` OR `textToFind`+`matchInstance` `` claims
  // `index`, `textToFind`, and `matchInstance` this way. The annotation is
  // mandatory, not just the colon: a bare label followed by a comma-joined
  // list is exactly how a response-fields bullet reads (`` Returns: `id`,
  // `name` ``), and reading that as a parameter claim would fail the build on
  // output field names the schema never had. Every targeting bullet in the
  // docs carries an annotation, so requiring one costs no coverage.
  // Requiring at least one real separator right after the prefix is what
  // keeps this from firing on an ordinary description bullet like `` Inline
  // images are surfaced (not dropped): `markdown` renders ... `` (one name,
  // then prose, no separator).
  //
  // Both shapes only ever capture a leading run: whatever comes after the
  // last separator this gate recognizes goes unread, and any bullet whose
  // backticks never follow one of these two shapes (a free-form sentence that
  // merely mentions a name in passing) is skipped entirely. That is the blind
  // spot that remains. This only checks the direction that
  ...
  it('never claims a parameter that is not in the tool\'s schema', () => {
    ...
    // `with`. The annotation is required (a bare `Returns:` label does not
    // qualify), and so is a real separator between the first and second name:
    // a free-form bullet that opens with a label, a colon, and a single
    // backticked name (then prose) is not a claim.
    const paramToken = '`[A-Za-z0-9_]+`(?:\\s*\\([^()]*\\))?';
    const paramSep = '\\s*(?:OR|or|with|\\+|,|/)\\s*';
    ...
    for (const line of toolReference.split(/\r?\n/)) {
      ...
      // A name's trailing parenthetical is an annotation, not a claim: it can
      // carry enum members (`` `position` (`before`/`after`) `` on
      // `insertText`), so it is dropped before the names are read.
      const claimedNames = claimText.replace(/\([^()]*\)/g, '');
      for (const [, name] of claimedNames.matchAll(/`([A-Za-z0-9_]+)`/g)) {
        if (!schema.has(name)) {
