...
export class AuthServer {
  ...
  /** Build the Google consent URL for a client using this server's scopes/prompt. */
  private buildConsentUrl(client: OAuth2Client): string {
  ...
  private setupRoutes(): void {
    this.app.get('/', (req, res) => {
      ...
      const authUrl = this.buildConsentUrl(clientForUrl);
      // escapeHtml guards attribute breakout here, not the URL scheme; this
      // is safe only because authUrl is generated by google-auth-library
      // (always an https:// Google URL), never derived from user input.
  ...
  async start(openBrowser = true): Promise<boolean> {
    ...
    if (openBrowser) {
      ...
      const authorizeUrl = this.buildConsentUrl(this.flowOAuth2Client);
  ...
  public getAuthorizeUrl(): string | null {
    ...
    return this.buildConsentUrl(this.flowOAuth2Client);
  }
